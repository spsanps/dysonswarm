const canvas = document.getElementById('canvas');
    ...
    pauseButton.addEventListener('click', () => { paused = !paused; needsRedraw = true; updatePauseButton(); });
    slider.addEventListener('input', () => { document.getElementById('disassemblyValue').textContent = slider.value + '%'; });
    updatePauseButton();
    ...
    function update(dt) {
      ...
      if (dt > 0 && Math.random() < debrisRate && getMercurySize() > 0.5) {
    ...
    resize();
    init();
    window.addEventListener('resize', () => { resize(); generateStars(); needsRedraw = true; });

    let lastTime = 0;
    function animate(timestamp) {
      const dt = Math.min(50, timestamp - lastTime);
      lastTime = timestamp;
      if (document.hidden || (paused && !needsRedraw)) { requestAnimationFrame(animate); return; }
      needsRedraw = false;
      if (!paused) time += dt;
